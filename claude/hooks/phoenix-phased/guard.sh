...
if [ "$TOOL" = "Bash" ]; then
  ...
  if [ "$PHASE" = "executing" ] && echo "$CMD" | grep -Eq '(git[[:space:]]+checkout[[:space:]]+-b|git[[:space:]]+switch[[:space:]]+-c)[[:space:]].*-phase-[0-9]'; then
    HEAD_TS=$(git -C /Users/michaeldulle/Sites/phoenix log -1 --format=%ct 2>/dev/null || echo 0)
...
if [ "$TOOL" = "Edit" ] || [ "$TOOL" = "Write" ] || [ "$TOOL" = "NotebookEdit" ]; then
  ...
  case "$FP" in
    /Users/michaeldulle/Sites/phoenix/.workflow/*) exit 0 ;;   # plan docs, session files
    /Users/michaeldulle/Sites/phoenix/*)
