...
| `autoupdate.sh` | Scheduled-only, not run by hand: daily `git pull --ff-only` + `install.sh` relink. Skips the pull (and logs a warning to `~/.dotfiles-autoupdate.log`) if the working tree is dirty. Deliberately doesn't run `provision.sh` (formula/cask installs, macOS defaults) unattended; re-run that by hand after a provisioning change. Scheduled daily at 23:45 via `provisioning/mac/launchagents/com.dotfiles.autoupdate.plist` on mac and `provisioning/linux/crons.sh` on Linux — takes effect only after `provision.sh`/`launchagents.sh` next runs on a given machine. |
